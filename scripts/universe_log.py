...
tH0 = np.array(
    [
        scint.quad(lambda a_bar: 1 / np.sqrt(density_bracket(a_bar, k_sign[l_])), 0, 1)[
            0
        ]
        for l_ in range(3)
    ]
...
def abar_derivative(t_tilde, a_bar, l_):
    return tH0[l_] * np.exp(t_tilde) * np.sqrt(density_bracket(a_bar, k_sign[l_]))
...
for l_ in range(3):
    plt.plot(
        np.exp(t_tilde[:, l_]), a_bar[:, l_], label=r"$ k = {} $".format(k_sign[l_])
    )
plt.xlabel(r"$ \bar{t} $")
plt.ylabel(r"$ \bar{a} $")
...
O_k = np.zeros((n, 3))
for l_ in range(3):
    O_k[:, l_] = k_sign[l_] * O_k0_abs / density_bracket(a_bar[:, l_], k_sign[l_])
