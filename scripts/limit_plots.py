...
for i in range(len(x[:-1])):
    if not found_17 and l17[i + 1] == l17[i]:
        print(
            "l17 transition point: a/a0 = {} in x[{}] and H/H0 = {}".format(
                np.exp(x[i]), i, l17[i]
            )
        )
    ...
    if not found_22 and l22[i + 1] == l22[i]:
        print(
            "l22 transition point: a/a0 = {} in x[{}] and H/H0 = {}".format(
                np.exp(x[i]), i, l22[i]
            )
        )
        print("ln(a/a0) = {}".format(x[i]))
    ...
    if not found_27 and l27[i + 1] == l27[i]:
        print(
            "l27 transition point: a/a0 = {} in x[{}] and H/H0 = {}".format(
                np.exp(x[i]), i, l27[i]
            )
        )
        print("ln(a/a0) = {}".format(x[i]))
...
plt.plot(
    np.exp(x), l17, label=r"$ l = 1 \times 10^{-17} $", linestyle="dotted", color="k"
)
plt.plot(
    np.exp(x), l22, label=r"$ l = 1 \times 10^{-22} $", linestyle="dashed", color="k"
)
plt.plot(
    np.exp(x), l27, label=r"$ l = 1 \times 10^{-27} $", linestyle="dashdot", color="k"
)
plt.plot(
    np.exp(gr_integration[:, 0]),
    gr_integration[:, 1],
    linestyle="solid",
    label="RG",
    color="k",
