...
do k = 1, size(lt)
  ...
  do j = 1, size(pt)
    ...
    do n_c = 1, size(slowroll_data, 1)
      if ((-60.0_qp <= slowroll_data(n_c, 1)) .and. (slowroll_data(n_c, 1) <= -50.0_qp)) then
        if ((slowroll_data(n_c, 4) >= e2_min) .and. (slowroll_data(n_c, 4) <= e2_max)) then
          in_sr_range(i, j, k, 2) = .true.
          exit
        end if
      end if
    end do
    ...
    do n_c = 1, size(slowroll_data, 1)
      if ((-60.0_qp <= slowroll_data(n_c, 1)) .and. (slowroll_data(n_c, 1) <= -50.0_qp)) then
        if ((slowroll_data(n_c, 5) >= e3_min) .and. (slowroll_data(n_c, 5) <= e3_max)) then
          in_sr_range(i, j, k, 3) = .true.
          exit
        end if
      end if
    end do
  ...
  do j = 1, size(pt)
  do k = 1, size(lt)
    if ((in_sr_range(i, j, k, 1)) &
      .and. (in_sr_range(i, j, k, 2)) &
      .and. (in_sr_range(i, j, k, 3))) then
      print *, "m =", mt(i)
      print *, "p =", pt(j)
      print *, "l =", lt(k)
      print *, "-------------"
