program find_solutions_exp
  ...
  !! Initial [[gi_exp_friedmann:x]] value
  real(qp), parameter :: xf = -100.0_qp
  !! Final [[gi_exp_friedmann:y]] value
  real(qp), parameter :: yi = 1.0_qp
  !! Initial [[gi_exp_friedmann:y]] value
...
! ---
  ...
  do k = 1, 3
    ...
    y(:, k) = gi_exp_solution(x, yi, conditions)

    sr_n_0(:, k, :) = gi_exp_sr0(x, y(:, k), conditions%l)
    sr1(:, k) = gi_exp_sr(sr_n_0(:, k, 1), sr_n_0(:, k, 2))
    sr2(:, k) = gi_exp_sr(sr_n_0(:, k, 1), sr1(:, k))
    sr3(:, k) = gi_exp_sr(sr_n_0(:, k, 1), sr2(:, k))
...
end program find_solutions_exp
